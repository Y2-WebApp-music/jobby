...
interface AchievementDialogProps {
  open: boolean;
  initialItem?: AchievementItem | null;
  onClose: () => void;
...
function DatePickerField({
  ...
  const minDateValue = parseToDate(minDate ?? "");
  const maxDateValue = parseToDate(maxDate ?? "");
...
export default function AchievementDialog({
  ...
  const [draft, setDraft] = useState<AchievementItem>(createEmptyAchievement());
  const isEditMode = Boolean(initialItem);

  useEffect(() => {
    if (!open) return;
    setDraft(initialItem ?? createEmptyAchievement());
  }, [open, initialItem]);
  ...
  const handleImageUpload = (files: FileList | null) => {
    if (!files) return;
    const left = MAX_ACHIEVEMENT_IMAGES - draft.images.length;
    if (left <= 0) return;
      ...
      >
        <div className="mb-4 flex items-start justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            aria-label="Close achievement form"
        ...
        <form onSubmit={handleSaveDraft} className="space-y-3">
          ...
          <div>
            <label className="mb-1 block text-sm text-slate-700">
              Upload Image (Up to {MAX_ACHIEVEMENT_IMAGES})
            </label>
            <input
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              {draft.images.length < MAX_ACHIEVEMENT_IMAGES ? (
                <label
